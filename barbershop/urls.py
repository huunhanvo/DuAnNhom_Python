...
urlpatterns = [
    ...
    # Include app URLs (order matters - more specific patterns first)
    # Note: Each app defines its own URL prefixes internally
    path('', include('core.urls')),           # Public pages + customer/admin/staff dashboards - MUST BE FIRST
    path('', include('accounts.urls')),       # Customer auth (accounts/*) + Staff management (admin/staff/*, staff/*)
    path('', include('services.urls')),       # Services management
    path('', include('bookings.urls')),       # Bookings (bookings/*, admin/bookings/*, staff/*, api/*)
    path('', include('attendance.urls')),     # Schedule & attendance
    path('', include('reports.urls')),        # Reports
    path('', include('reviews.urls')),        # Reviews
    
    # Staff/Admin Auth URLs (for admin and staff only)
    path('login/', views.login_view, name='staff_login'),  # Default login for staff/admin
